...
/**
 * BLOCKED run을 사람이 확인한 뒤 재개하는 오케스트레이션(fix round 2, P2a T7 재리뷰) —
 * {@link GateService#approve}와 정확히 같은 패턴을 그대로 미러링한다: continuation을
 * 먼저 만들고(BLOCKED에서만 legal하므로 순서가 중요하다), 그다음 원 run을
 * {@link Run#cancelWithNote}로 닫는다 — BLOCKED에 그대로 두면 {@code
 * RunService.ACTIVE_STATUSES}에 BLOCKED가 포함돼 있어(fix round 1, F2b) 다음 Dispatcher
 * 픽업의 "활성 run 중복" 가드와 동시성 집계에 계속 걸리기 때문이다.
 *
 * <p>이 재개는 게이트(사람 승인 요청) 없이 이뤄진다 — BLOCKED는 재시도 한도 소진으로
 * 시스템이 스스로 만든 정지 상태이지 사람에게 승인을 요청한 게 아니므로, {@link Gate}
 * 엔티티를 만들지 않는다(게이트 승인/거절과는 별개 경로).
 *
...
public class RunResumeService {
    ...
    /** 재조회 시 BLOCKED가 아니면(예: 이미 다른 경로로 재개/취소됨) 409 — reload 가드. */
    private Run loadBlockedRun(long runId) {
        Run run = runRepository.findById(runId)
                .orElseThrow(() -> new NotFoundException("run을 찾을 수 없습니다: " + runId));
        if (run.getStatus() != RunStatus.BLOCKED) {
            throw new ConflictException("BLOCKED 상태가 아닌 run은 재개할 수 없습니다(현재: "
